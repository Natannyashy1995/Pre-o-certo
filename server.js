...
// Seed manual (botão no painel admin)
app.post('/api/admin/seed-produtos', adminAuth, async (req, res) => {
  try {
    const existentes = await Produto.find({}, 'nome');
    const nomesSet   = new Set(existentes.map(p => p.nome.toLowerCase().trim()));
    const novos      = PRODUTOS_SEED.filter(p => !nomesSet.has(p.nome.toLowerCase().trim()));
    if (novos.length) await Produto.insertMany(novos);
    const total = await Produto.countDocuments({ ativo:true });
    res.json({ mensagem:`${novos.length} produtos adicionados`, novos:novos.length, total });
  } catch(e) { res.status(500).json({ erro: e.message }); }
});
