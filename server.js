...
async function seedInicial() {
  try {
    ...
    // Produtos — sempre verifica e adiciona os que faltam (não apenas quando vazio)
    ...
    if (novos.length) {
      ...
      console.log(`✅ +${novos.length} produtos adicionados (${existentes.length} já existiam)`);
    } else {
      console.log(`✅ Catálogo completo: ${existentes.length} produtos no banco`);
    }
...
const PRODUTOS_SEED = [
  ...
  // ── PRODUTOS ADICIONAIS (catálogo completo) ──────────────
  // Frutas extras
