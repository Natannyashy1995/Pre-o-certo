/**
 * PreçoCerto — Backend v7
 * ════════════════════════════════════════════════════════════
 * CORREÇÕES v7 (aplicadas sobre v6):
 *  1. Hard delete real de clientes (era soft delete com banPermanente)
 *  2. Sistema de Blacklist: números excluídos bloqueados por 2 meses
 *  3. Cron automático 24h para limpar blacklist vencida
 *  4. Cadastro verifica blacklist (não mais o cliente deletado)
 *  5. PromocaoSchema.produtoId aceita ObjectId OU id numérico local
 *  6. Validação de promoção corrigida (não rejeita mais produtos do catálogo)
 *  7. Rotas admin: alterar senha de cliente, editar dados de cliente
 *  8. Rotas CRUD completas da blacklist para o painel admin
 *  9. Audit log de ações administrativas
 * ════════════════════════════════════════════════════════════
 */
...
// Resend API (HTTP — funciona no Render gratuito)
...
const EMAIL_FROM     = process.env.EMAIL_FROM || 'PreçoCerto <onboarding@resend.dev>';
...
async function enviarEmail(para, assunto, html) {
  if (!RESEND_API_KEY) {
    console.log(`[EMAIL SIMULADO] Para: ${para} | ${assunto}`);
    return false;
  }
  try {
    ...
    console.error('❌ Erro email Resend:', e.message);
    return false;
  }
}
...
const PromocaoSchema = new mongoose.Schema({
  produtoId:  { type: mongoose.Schema.Types.Mixed, default: null }, // ObjectId do banco OU id numérico local
  mercadoId:  { type: mongoose.Schema.Types.ObjectId, ref: 'Mercado', required: true },
...
const BlacklistSchema = new mongoose.Schema({
  ...
  motivo:         { type: String, default: 'Conta excluída por administrador' },
  criadoPor:      { type: String, default: 'admin' },
  ativo:          { type: Boolean, default: true }
});
BlacklistSchema.index({ telefone: 1 });
BlacklistSchema.index({ dataVencimento: 1 });

// ─── AUDIT LOG ─────────────────────────────────────────────────────────────
const AdminAuditSchema = new mongoose.Schema({
  adminUsuario: { type: String, required: true },
  acao:         { type: String, required: true },
  alvoTipo:     { type: String, default: '' },
  alvoId:       { type: String, default: '' },
  alvoIdent:    { type: String, default: '' },
...
// ─── HELPERS BLACKLIST ─────────────────────────────────────────────────────
const normTel = tel => String(tel||'').replace(/\D/g,'');

async function telNaBlacklist(telefone) {
  const agora = new Date();
  return await Blacklist.findOne({
    telefone: normTel(telefone), ativo: true, dataVencimento: { $gt: agora }
  }) || null;
}

async function limparBlacklistVencida() {
  try {
    const r = await Blacklist.updateMany(
      { dataVencimento: { $lte: new Date() }, ativo: true },
      { ativo: false }
    );
    if (r.modifiedCount > 0)
      console.log(`[Blacklist] ${r.modifiedCount} entrada(s) vencida(s) desativada(s)`);
  } catch(e) { console.error('[Blacklist cron]', e.message); }
}

async function auditLog(adminUsuario, acao, alvoTipo='', alvoId='', alvoIdent='', detalhes=null) {
  try { await AdminAudit.create({ adminUsuario, acao, alvoTipo, alvoId, alvoIdent, detalhes }); } catch(e) {}
}
...
app.post('/api/auth/cadastro', async (req, res) => {
  try {
    ...
    if (await Cliente.findOne({ login }))
      return res.status(409).json({ erro: 'Login já está em uso' });
    if (await Cliente.findOne({ telefone: telNorm }))
      return res.status(409).json({ erro: 'Este número de WhatsApp já está cadastrado' });

    // CORREÇÃO BUG 1: verificar blacklist (números de contas excluídas ficam aqui por 2 meses)
    const bloqueio = await telNaBlacklist(telNorm);
    if (bloqueio) {
      const dias = Math.max(0, Math.ceil((bloqueio.dataVencimento - new Date()) / 86400000));
      return res.status(403).json({
        erro: `Este número está temporariamente impedido de criar nova conta. Aguarde ${dias} dia(s).`,
        diasRestantes: dias
      });
    }
    ...
    res.status(201).json({ token, nome: cliente.nome, login: cliente.login, telefone: telNorm });
  } catch(e) { res.status(500).json({ erro: e.message }); }
});
...
app.post('/api/promocoes', adminAuth, async (req, res) => {
  try {
    ...
    // mercadoId sempre precisa ser ObjectId real do banco
    if (!mercadoId || !isObjId(String(mercadoId)))
      return res.status(400).json({ erro: 'Mercado inválido — selecione um mercado cadastrado no banco' });

    // CORREÇÃO BUG 3: produtoId pode ser ObjectId do MongoDB OU id numérico local
    if (!produtoId)
      return res.status(400).json({ erro: 'Produto inválido — selecione um produto' });

    // Se for ObjectId, verificar existência no banco
    if (isObjId(String(produtoId))) {
      const existe = await Produto.findById(produtoId);
      if (!existe) return res.status(400).json({ erro: 'Produto não encontrado no catálogo' });
    }
    // Id numérico (dados locais) — aceitar sem validação de banco
...
app.patch('/api/admin/solicitacoes/:id/aprovar', adminAuth, async (req, res) => {
  try {
    ...
    // Gerar link WhatsApp com credenciais
    const msgWhats = encodeURIComponent(
      `🎉 Olá ${sol.responsavel}! Seu mercado *${sol.mercado}* foi aprovado no PreçoCerto!\n\n` +
      `✅ *Plano:* ${sol.plano}\n` +
      `👤 *Login:* ${loginGerado}\n` +
      `🔑 *Senha:* ${senhaGerada}\n\n` +
      `Acesse: ${APP_URL}\n` +
    ...
    const whatsappLink = sol.whatsapp
      ? `https://wa.me/55${sol.whatsapp.replace(/\D/g,'')}?text=${msgWhats}`
      : null;
...
// Alterar senha de cliente pelo admin (sem precisar da senha atual)
app.patch('/api/admin/clientes/:id/senha', adminAuth, async (req, res) => {
  try {
    ...
    if (!cliente) return res.status(404).json({ erro: 'Cliente não encontrado' });
    await Cliente.updateOne({ _id: cliente._id }, { senhaHash: await bcrypt.hash(novaSenha, 10) });
    await auditLog(req.user.usuario, 'ALTERAR_SENHA_CLIENTE', 'cliente', String(cliente._id), cliente.telefone);
    res.json({ mensagem: 'Senha alterada com sucesso' });
  } catch(e) { res.status(500).json({ erro: e.message }); }
});
...
app.patch('/api/admin/clientes/:id/editar', adminAuth, async (req, res) => {
  try {
    ...
    const cliente = await Cliente.findByIdAndUpdate(req.params.id, upd, { new: true }).select('-senhaHash');
    if (!cliente) return res.status(404).json({ erro: 'Cliente não encontrado' });
    await auditLog(req.user.usuario, 'EDITAR_CLIENTE', 'cliente', String(cliente._id), cliente.telefone, upd);
    res.json({ mensagem: 'Dados atualizados', cliente });
  } catch(e) { res.status(500).json({ erro: e.message }); }
});

// CORREÇÃO BUG 1: hard delete real + blacklist 2 meses
app.delete('/api/admin/clientes/:id', adminAuth, async (req, res) => {
  try {
    ...
    const telefone = normTel(cliente.telefone);
    const motivo   = req.body?.motivo || 'Conta excluída por administrador';

    // 1. Hard delete — remove o documento de verdade
    await Cliente.findByIdAndDelete(req.params.id);

    // 2. Blacklist por 2 meses (upsert: renova se já existir)
    const dataVencimento = new Date();
    dataVencimento.setMonth(dataVencimento.getMonth() + 2);
    await Blacklist.findOneAndUpdate(
      { telefone },
      { telefone, dataInicio: new Date(), dataVencimento, motivo, criadoPor: req.user.usuario, ativo: true },
      { upsert: true, new: true }
    );

    // 3. Registros de auditoria
    await auditLog(req.user.usuario, 'EXCLUIR_CLIENTE', 'cliente', req.params.id, telefone, { nome: cliente.nome, motivo });
    await log('admin', `Cliente excluído: ${cliente.nome} (${telefone})`, req.user.usuario, getIP(req));

    res.json({ mensagem: `Conta de ${cliente.nome} excluída. Número ${telefone} bloqueado por 2 meses.` });
  } catch(e) { res.status(500).json({ erro: e.message }); }
});

// ═══════════════════════════════════════════════
// BLACKLIST (gestão admin)
// ═══════════════════════════════════════════════
app.get('/api/admin/blacklist', adminAuth, async (req, res) => {
  try {
    ...
    res.json(lista.map(r => ({
      ...r.toObject(),
      ativoReal: r.dataVencimento > agora,
      diasRestantes: Math.max(0, Math.ceil((r.dataVencimento - agora) / 86400000))
    })));
  } catch(e) { res.status(500).json({ erro: e.message }); }
});
...
app.delete('/api/admin/blacklist/:id', adminAuth, async (req, res) => {
  try {
    const entry = await Blacklist.findByIdAndUpdate(req.params.id, { ativo: false }, { new: true });
    if (!entry) return res.status(404).json({ erro: 'Registro não encontrado' });
    await auditLog(req.user.usuario, 'BLACKLIST_LIBERAR', 'blacklist', String(entry._id), entry.telefone);
    res.json({ mensagem: `Número ${entry.telefone} liberado` });
  } catch(e) { res.status(500).json({ erro: e.message }); }
});

app.patch('/api/admin/blacklist/:id/vencimento', adminAuth, async (req, res) => {
  try {
    const { novaData } = req.body;
    if (!novaData) return res.status(400).json({ erro: 'novaData obrigatória (YYYY-MM-DD)' });
    const data = new Date(novaData);
    if (isNaN(data)) return res.status(400).json({ erro: 'Data inválida' });
    const entry = await Blacklist.findByIdAndUpdate(req.params.id, { dataVencimento: data, ativo: true }, { new: true });
    if (!entry) return res.status(404).json({ erro: 'Registro não encontrado' });
    await auditLog(req.user.usuario, 'BLACKLIST_ALTERAR_DATA', 'blacklist', String(entry._id), entry.telefone);
    res.json({ mensagem: 'Data atualizada', entry });
  } catch(e) { res.status(500).json({ erro: e.message }); }
});

app.post('/api/admin/blacklist/limpar', adminAuth, async (req, res) => {
  try { await limparBlacklistVencida(); res.json({ mensagem: 'Entradas vencidas desativadas' }); }
  catch(e) { res.status(500).json({ erro: e.message }); }
});

app.get('/api/admin/audit', adminAuth, async (req, res) => {
  try { res.json(await AdminAudit.find().sort({ criadoEm: -1 }).limit(200)); }
  catch(e) { res.status(500).json({ erro: e.message }); }
});
...
app.listen(PORT, async () => {
  console.log(`🚀 PreçoCerto v7 rodando na porta ${PORT}`);
  if (MONGODB_URI) {
    setTimeout(seedInicial, 2000);
    // Cron blacklist: limpar vencidos ao iniciar e a cada 24h
    limparBlacklistVencida();
    setInterval(limparBlacklistVencida, 24 * 60 * 60 * 1000);
  }
});
