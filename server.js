...
app.post('/api/auth/cadastro', async (req, res) => {
  try {
    ...
    // Telefone e login obrigatórios
    if (!nome||!login||!senha||!telefone) return res.status(400).json({ erro: 'Nome, login, senha e telefone são obrigatórios' });
    // Normalizar telefone — só números
    const telNorm = telefone.replace(/\D/g,'');
    if (telNorm.length < 10) return res.status(400).json({ erro: 'Telefone inválido — informe DDD + número' });
    // Verificar duplicatas
    if (await Cliente.findOne({ login })) return res.status(409).json({ erro: 'Login já está em uso' });
    if (await Cliente.findOne({ telefone: telNorm })) return res.status(409).json({ erro: 'Este número de WhatsApp já está cadastrado' });
    // Verificar se número está banido
    const telBloqueado = await Cliente.findOne({ telefone: telNorm, $or: [{ bloqueado: true }, { banPermanente: true }] });
    if (telBloqueado) return res.status(403).json({ erro: 'Este número está bloqueado e não pode criar nova conta' });
    ...
    const cliente = await Cliente.create({
      ...
      aceitouTermos: true,
      emailVerificado: true, // sem verificação por email
      ip: getIP(req)
    });
