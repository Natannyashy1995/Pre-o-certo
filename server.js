...
app.post('/api/admin/limpar-duplicatas', adminAuth, async (req, res) => {
  try {
    ...
    // 1. Remove produtos cujo nome NÃO está no seed (versões antigas/obsoletas)
    const todos = await Produto.find({}, 'nome _id');
    const obsoletos = todos.filter(p => !nomesSeed.has(p.nome.toLowerCase().trim()));
    if (obsoletos.length) {
      await Produto.deleteMany({ _id: { $in: obsoletos.map(p => p._id) } });
      removidos += obsoletos.length;
    }

    // 2. Remove duplicatas — para cada nome do seed, mantém só 1 (o mais antigo)
    for (const seedProd of PRODUTOS_SEED) {
      const encontrados = await Produto.find({ nome: seedProd.nome }).sort({ createdAt: 1 });
      if (encontrados.length > 1) {
        const extras = encontrados.slice(1).map(p => p._id);
        await Produto.deleteMany({ _id: { $in: extras } });
        removidos += extras.length;
      }
    }

    // 3. Garante que todos os produtos do seed existem (insere faltantes)
    const restantes = await Produto.find({}, 'nome');
    const nomesRestantes = new Set(restantes.map(p => p.nome.toLowerCase().trim()));
    const faltantes = PRODUTOS_SEED.filter(p => !nomesRestantes.has(p.nome.toLowerCase().trim()));
    if (faltantes.length) {
      await Produto.insertMany(faltantes, { ordered: false }).catch(() => {});
    }

    const total = await Produto.countDocuments({ ativo: true });
    await registrarLog('admin', `Limpeza duplicatas: -${removidos} removidos. Total: ${total}`, req.user.usuario, getIP(req));
    res.json({ removidos, total, mensagem: `${removidos} produtos removidos. Total: ${total}` });
  } catch(e) { res.status(500).json({ erro: e.message }); }
});
