...
app.get('/api/fix-catalogo', async (req, res) => {
  ...
  try {
    ...
    // Produtos que faltam no banco
    const novos = PRODUTOS_SEED.filter(p => !nomesTodos.has(p.nome.toLowerCase().replace(/\s+/g,' ').trim()));

    // Inserir um por um para capturar erros individuais
    let inseridos = 0;
    const erros = [];
    ...
    <p>Faltavam: <b>${novos.length}</b></p>
    <p>Inseridos com sucesso: <b>${inseridos}</b></p>
    <p>Erros: <b>${erros.length}</b></p>
    <p>Total agora: <b>${total}</b></p>
    <p>Seed tem: <b>${PRODUTOS_SEED.length}</b></p>
    ${erros.length ? '<hr><b>Erros detalhados:</b><pre>' + erros.slice(0,10).join('\n') + '</pre>' : ''}`;
    res.send(html);
