...
app.post('/api/ia/analisar', async (req, res) => {
  ...
  try {
    ...
    const modelos = ['gemini-2.0-flash-lite', 'gemini-2.0-flash', 'gemini-1.5-flash-latest'];
    let ultimoErro = '';
    for (const modelo of modelos) {
      const r = await fetch(`https://generativelanguage.googleapis.com/v1/models/${modelo}:generateContent?key=${GEMINI_KEY}`, {
