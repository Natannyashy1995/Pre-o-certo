...
async function seedInicial() {
  try {
    ...
    if (await Produto.countDocuments() === 0) {
      ...
      console.log(`✅ ${PRODUTOS_SEED.length} produtos inseridos`);
    } else {
      // Adicionar produtos que faltam sem duplicar
      const existentes = await Produto.find({}, 'nome');
      const nomesSet   = new Set(existentes.map(p => p.nome.toLowerCase().trim()));
      const novos      = PRODUTOS_SEED.filter(p => !nomesSet.has(p.nome.toLowerCase().trim()));
      if (novos.length) { await Produto.insertMany(novos); console.log(`✅ +${novos.length} produtos novos adicionados`); }
    }
...
const PRODUTOS_SEED = [
  ...
  // ☕ CAFÉ — inclui marcas locais de Piatã
...
app.post('/api/admin/seed-produtos', adminAuth, async (req, res) => {
  try {
    ...
    const existentes = await Produto.find({}, 'nome');
    const nomesSet   = new Set(existentes.map(p => p.nome.toLowerCase().replace(/\s+/g,' ').trim()));
    const novos      = PRODUTOS_SEED.filter(p => !nomesSet.has(p.nome.toLowerCase().replace(/\s+/g,' ').trim()));
    let inseridos = 0;
    if (novos.length) {
    ...
    const total = await Produto.countDocuments({ ativo: true });
    await registrarLog('admin', `Seed produtos: +${inseridos} novos. Total: ${total}`, req.user.usuario, getIP(req));
    res.json({
      mensagem: inseridos > 0 ? `+${inseridos} produtos adicionados ao banco!` : 'Catálogo já estava completo — nenhum produto novo.',
      novos: inseridos,
      total,
