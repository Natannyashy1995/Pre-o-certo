...
app.get('/api/fix-catalogo', async (req, res) => {
  ...
  try {
    ...
    const nomesTodosNorm = new Set(todos.map(p => norm(p.nome)));

    // Produtos do seed que NAO existem no banco (comparação com normalização de acentos)
    const novos = PRODUTOS_SEED.filter(p => !nomesTodosNorm.has(norm(p.nome)));
    ...
    for (const p of novos) {
      try {
        await Produto.create({ nome: p.nome, emoji: p.emoji, categoria: p.categoria, ativo: true });
        inseridos++;
      } catch(e2) {
        erros.push(`${p.nome}: ${e2.message}`);
      }
    }

    const total = await Produto.countDocuments({ ativo: true });

    // Mostra primeiros 20 nomes do banco para diagnóstico
    const amostraBanco = ativos.slice(0,20).map(p => `"${p.nome}" → norm:"${norm(p.nome)}"`).join('<br>');
    // Mostra primeiros 5 do seed que "já existiam" antes desta rodada
    const seedNorm = PRODUTOS_SEED.slice(0,5).map(p => `"${p.nome}" → norm:"${norm(p.nome)}"`).join('<br>');

    const html = `<h2>Fix Catálogo</h2>
    <p>Faltavam (antes): <b>${novos.length}</b></p>
    <p>Inseridos agora: <b>${inseridos}</b></p>
    <p>Erros: <b>${erros.length}</b></p>
    <p>Total banco agora: <b>${total}</b></p>
    <p>Seed tem: <b>${PRODUTOS_SEED.length}</b></p>
    ${erros.length ? '<hr><b>Erros:</b><pre>' + erros.slice(0,5).join('\n') + '</pre>' : ''}
    <hr><b>Amostra banco (primeiros 20 ativos):</b><br>${amostraBanco}
    <hr><b>Amostra seed (primeiros 5):</b><br>${seedNorm}`;
    res.send(html);
  } catch(e) { res.status(500).send('Erro geral: ' + e.message); }
});
