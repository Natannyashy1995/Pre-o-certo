...
app.post('/api/precos', authMiddleware, async (req, res) => {
  try {
    ...
    // Validate ObjectIds
    if (!mongoose.Types.ObjectId.isValid(produtoId)) return res.status(400).json({ erro: 'Produto inválido — recadastre o produto pelo painel admin' });
    if (!mongoose.Types.ObjectId.isValid(mercadoId)) return res.status(400).json({ erro: 'Mercado inválido — o mercado não foi salvo no banco. Recadastre pelo painel admin' });
...
// Seed extra produtos (chamado uma vez para completar catálogo)
app.post('/api/admin/seed-produtos', adminAuth, async (req, res) => {
  try {
    const extraProdutos = [
      // Mercearia extra
      {nome:'Macarrão Espaguete Adria 500g',emoji:'🍝',categoria:'Mercearia'},{nome:'Macarrão Parafuso Adria 500g',emoji:'🍝',categoria:'Mercearia'},{nome:'Macarrão Penne Barilla 500g',emoji:'🍝',categoria:'Mercearia'},{nome:'Arroz Tio João 5kg',emoji:'🍚',categoria:'Mercearia'},{nome:'Arroz Camil 2kg',emoji:'🍚',categoria:'Mercearia'},{nome:'Feijão Fradinho Camil 1kg',emoji:'🫘',categoria:'Mercearia'},{nome:'Feijão Verde Camil 1kg',emoji:'🫘',categoria:'Mercearia'},{nome:'Lentilha Camil 500g',emoji:'🫘',categoria:'Mercearia'},{nome:'Grão de Bico Camil 500g',emoji:'🫘',categoria:'Mercearia'},{nome:'Aveia Quaker 500g',emoji:'🌾',categoria:'Mercearia'},{nome:'Granola 3Corações 500g',emoji:'🌾',categoria:'Mercearia'},{nome:'Azeite Carbonell 500ml',emoji:'🫒',categoria:'Mercearia'},{nome:'Molho Shoyu Kikkoman 150ml',emoji:'🍶',categoria:'Mercearia'},{nome:'Molho Inglês Worcestershire 150ml',emoji:'🍶',categoria:'Mercearia'},{nome:'Maionese Hellmanns 250g',emoji:'🥛',categoria:'Mercearia'},{nome:'Maionese Hellmanns 500g',emoji:'🥛',categoria:'Mercearia'},{nome:'Ketchup Heinz 397g',emoji:'🍅',categoria:'Mercearia'},{nome:'Mostarda Hemmer 200g',emoji:'🌻',categoria:'Mercearia'},{nome:'Creme de Cebola Knorr 34g',emoji:'🧅',categoria:'Mercearia'},{nome:'Tempero Baiano Kitano 40g',emoji:'🌶️',categoria:'Mercearia'},{nome:'Pimenta do Reino Ducros 30g',emoji:'🫙',categoria:'Mercearia'},{nome:'Canela em Pó 30g',emoji:'🍂',categoria:'Mercearia'},{nome:'Cravo da Índia 10g',emoji:'🌿',categoria:'Mercearia'},{nome:'Fermento Pó Royal 100g',emoji:'🧁',categoria:'Mercearia'},{nome:'Bicarbonato de Sódio 200g',emoji:'⚗️',categoria:'Mercearia'},{nome:'Gelatina Incolor 12g',emoji:'🫙',categoria:'Mercearia'},{nome:'Amido de Milho Maizena 500g',emoji:'🌽',categoria:'Mercearia'},{nome:'Tapioca Yoki 500g',emoji:'🫓',categoria:'Mercearia'},{nome:'Cuscuz Flocão 500g',emoji:'🌾',categoria:'Mercearia'},{nome:'Leite de Coco Sococo 200ml',emoji:'🥥',categoria:'Mercearia'},{nome:'Extrato de Coco 200ml',emoji:'🥥',categoria:'Mercearia'},{nome:'Milho Cozido Lata Bonduelle 200g',emoji:'🌽',categoria:'Mercearia'},{nome:'Ervilha Lata Bonduelle 200g',emoji:'🟢',categoria:'Mercearia'},{nome:'Palmito Pupunha 300g',emoji:'🌿',categoria:'Mercearia'},{nome:'Azeitona Verde Predilecta 150g',emoji:'🫒',categoria:'Mercearia'},{nome:'Sardinha Lata Coqueiro 125g',emoji:'🐟',categoria:'Mercearia'},{nome:'Atum Lata Gomes da Costa 170g',emoji:'🐟',categoria:'Mercearia'},
      // Laticínios extra
      {nome:'Iogurte Grego Danone 100g',emoji:'🥛',categoria:'Laticínios'},{nome:'Bebida Láctea Toddynho 200ml',emoji:'🥛',categoria:'Laticínios'},{nome:'Leite Fermentado Yakult 80ml',emoji:'🥛',categoria:'Laticínios'},{nome:'Queijo Ricota kg',emoji:'🧀',categoria:'Laticínios'},{nome:'Queijo Cottage 200g',emoji:'🧀',categoria:'Laticínios'},{nome:'Nata 200g',emoji:'🥛',categoria:'Laticínios'},
      // Bebidas extra  
      {nome:'Energético Red Bull 250ml',emoji:'⚡',categoria:'Bebidas'},{nome:'Energético Monster 473ml',emoji:'⚡',categoria:'Bebidas'},{nome:'Isotônico Gatorade 500ml',emoji:'💧',categoria:'Bebidas'},{nome:'Suco Integral Del Valle 1L',emoji:'🧃',categoria:'Bebidas'},{nome:'Néctar Tial 1L',emoji:'🧃',categoria:'Bebidas'},{nome:'Refrigerante Fanta 2L',emoji:'🥤',categoria:'Bebidas'},{nome:'Refrigerante Sprite 2L',emoji:'🥤',categoria:'Bebidas'},{nome:'Refrigerante Kuat 2L',emoji:'🥤',categoria:'Bebidas'},{nome:'Água Tônica Antarctica 350ml',emoji:'💧',categoria:'Bebidas'},{nome:'Leite de Caixinha Piracanjuba 200ml',emoji:'🥛',categoria:'Bebidas'},{nome:'Vinho Branco Suave 720ml',emoji:'🍷',categoria:'Bebidas'},{nome:'Cerveja Original Long Neck 355ml',emoji:'🍺',categoria:'Bebidas'},{nome:'Cachaça 51 965ml',emoji:'🍶',categoria:'Bebidas'},
      // Higiene extra
      {nome:'Shampoo TRESemmé 400ml',emoji:'🧴',categoria:'Higiene'},{nome:'Condicionador TRESemmé 400ml',emoji:'🧴',categoria:'Higiene'},{nome:'Shampoo Elseve 400ml',emoji:'🧴',categoria:'Higiene'},{nome:'Sabonete Líquido Dove 250ml',emoji:'🧴',categoria:'Higiene'},{nome:'Sabonete Líquido Lux 250ml',emoji:'🧴',categoria:'Higiene'},{nome:'Protetor Solar Banana Boat FPS50 200ml',emoji:'☀️',categoria:'Higiene'},{nome:'Fio Dental Oral-B 50m',emoji:'🦷',categoria:'Higiene'},{nome:'Enxaguante Bucal Listerine 250ml',emoji:'🦷',categoria:'Higiene'},{nome:'Creme para Mãos Neutrogena 56g',emoji:'🧴',categoria:'Higiene'},{nome:'Talco Johnson 200g',emoji:'☁️',categoria:'Higiene'},{nome:'Xampu Johnson Baby 200ml',emoji:'🧴',categoria:'Higiene'},{nome:'Sabonete Johnson Baby 80g',emoji:'🧼',categoria:'Higiene'},{nome:'Creme Bepantol 30g',emoji:'🧴',categoria:'Higiene'},
      // Limpeza extra
      {nome:'Lava Louças Ypê Gel 500ml',emoji:'🫧',categoria:'Limpeza'},{nome:'Desengordurante Bombril 500ml',emoji:'🧹',categoria:'Limpeza'},{nome:'Pastilha Sanitária Pato 25g',emoji:'🚽',categoria:'Limpeza'},{nome:'Luva Látex Mucambo M un',emoji:'🧤',categoria:'Limpeza'},{nome:'Balde 10L un',emoji:'🪣',categoria:'Limpeza'},{nome:'Pano Úmido Huggies 48un',emoji:'🧻',categoria:'Limpeza'},
      // Cosméticos extra
      {nome:'Creme Facial Ponds 100g',emoji:'🧴',categoria:'Cosméticos'},{nome:'Sérum Vitamina C L\'Oréal 30ml',emoji:'✨',categoria:'Cosméticos'},{nome:'Máscara Facial Dermage un',emoji:'🎭',categoria:'Cosméticos'},{nome:'Perfume Boticário un',emoji:'🌺',categoria:'Cosméticos'},{nome:'Descolorante Emulsão L\'Oréal un',emoji:'🎨',categoria:'Cosméticos'},{nome:'Cera Depilatória Veet un',emoji:'🌿',categoria:'Cosméticos'},{nome:'Esfoliante Facial Neutrogena 100g',emoji:'🧴',categoria:'Cosméticos'},{nome:'Batom Líquido Matte Vult un',emoji:'💋',categoria:'Cosméticos'},{nome:'Corretivo Ruby Rose un',emoji:'🧴',categoria:'Cosméticos'},{nome:'Fixador de Maquiagem MAC un',emoji:'✨',categoria:'Cosméticos'},
      // Padaria extra
      {nome:'Croissant Congelado 6un',emoji:'🥐',categoria:'Padaria'},{nome:'Pão de Queijo Congelado 400g',emoji:'🧀',categoria:'Padaria'},{nome:'Bolo de Cenoura Caseiro un',emoji:'🎂',categoria:'Padaria'},{nome:'Biscoito Amanteigado 400g',emoji:'🍪',categoria:'Padaria'},
      // Açougue extra
      {nome:'Carne Seca kg',emoji:'🥩',categoria:'Açougue'},{nome:'Charque kg',emoji:'🥩',categoria:'Açougue'},{nome:'Mocotó kg',emoji:'🦴',categoria:'Açougue'},{nome:'Fígado Bovino kg',emoji:'🥩',categoria:'Açougue'},{nome:'Peixe Badejo kg',emoji:'🐟',categoria:'Açougue'},{nome:'Peixe Corvina kg',emoji:'🐟',categoria:'Açougue'},{nome:'Camarão Seco 200g',emoji:'🦐',categoria:'Açougue'},
      // Congelados extra
      {nome:'Empanado Frango Sadia 300g',emoji:'🍗',categoria:'Congelados'},{nome:'Espetinho de Frango Congelado 500g',emoji:'🍢',categoria:'Congelados'},{nome:'Coxinha Congelada 500g',emoji:'🍗',categoria:'Congelados'},{nome:'Polpa de Fruta Goiaba 1kg',emoji:'🍈',categoria:'Congelados'},{nome:'Polpa de Fruta Maracujá 1kg',emoji:'🟡',categoria:'Congelados'},
      // Frutas extra
      {nome:'Coco Verde un',emoji:'🥥',categoria:'Frutas'},{nome:'Acerola kg',emoji:'🔴',categoria:'Frutas'},{nome:'Goiaba kg',emoji:'🟢',categoria:'Frutas'},{nome:'Maracujá kg',emoji:'🟡',categoria:'Frutas'},{nome:'Abacate kg',emoji:'🥑',categoria:'Frutas'},{nome:'Carambola kg',emoji:'⭐',categoria:'Frutas'},
      // Legumes extra
      {nome:'Inhame kg',emoji:'🍠',categoria:'Legumes'},{nome:'Cará kg',emoji:'🍠',categoria:'Legumes'},{nome:'Abóbora kg',emoji:'🎃',categoria:'Legumes'},{nome:'Berinjela kg',emoji:'🍆',categoria:'Legumes'},{nome:'Chuchu kg',emoji:'🥒',categoria:'Legumes'},{nome:'Jiló kg',emoji:'🟢',categoria:'Legumes'},{nome:'Vagem kg',emoji:'🌿',categoria:'Legumes'},
      // Verduras extra
      {nome:'Agrião maço',emoji:'🥬',categoria:'Verduras'},{nome:'Hortelã maço',emoji:'🌿',categoria:'Verduras'},{nome:'Manjericão maço',emoji:'🌿',categoria:'Verduras'},{nome:'Repolho Roxo un',emoji:'🥦',categoria:'Verduras'},
      // Utilidades extra
      {nome:'Vela Comum 8un',emoji:'🕯️',categoria:'Utilidades'},{nome:'Isqueiro BIC un',emoji:'🔥',categoria:'Utilidades'},{nome:'Pilha AAA Duracell 2un',emoji:'🔋',categoria:'Utilidades'},{nome:'Pilha 9V Duracell un',emoji:'🔋',categoria:'Utilidades'},{nome:'Saco Zip Lock 20un',emoji:'🫙',categoria:'Utilidades'},{nome:'Papel Manteiga 30cm 5m',emoji:'📄',categoria:'Utilidades'},{nome:'Palito de Dente 200un',emoji:'🪥',categoria:'Utilidades'},{nome:'Guardanapo 50un',emoji:'🧻',categoria:'Utilidades'},{nome:'Toalha de Papel Snob 2 rolos',emoji:'🧻',categoria:'Utilidades'}
    ];
    // Só insere os que não existem
    let adicionados = 0;
    for(const p of extraProdutos){
      const existe = await Produto.findOne({nome: p.nome});
      if(!existe){ await Produto.create(p); adicionados++; }
    }
    res.json({mensagem:`${adicionados} produtos adicionados!`, total: await Produto.countDocuments()});
  } catch(e){ res.status(500).json({erro: e.message}); }
});
