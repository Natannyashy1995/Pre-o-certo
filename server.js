    ...
    .then(() => {
      ...
      const limparDados48h = async () => {
        try {
          ...
          console.log('[Limpeza 48h] logs: ' + rLogs.deletedCount + ', solicitacoes: ' + rSolic.deletedCount);
        } catch(e) {
      ...
      const limparDados = async () => {
        try {
          ...
          // Remove logs com mais de 48h (exceto tipo 'admin' e 'auth' que ficam 7 dias)
          const limite7d = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
          const rLogs = await Log.deleteMany({
            $or: [
              { tipo: { $nin: ['admin','auth'] }, createdAt: { $lt: limite48h } },
          ...
          // Remove solicitações recusadas ou aprovadas com mais de 48h
          const rSolic = await Solicitacao.deleteMany({
            status: { $in: ['Aprovado','Recusado'] },
            updatedAt: { $lt: limite48h }
          });
      ...
      setTimeout(limparDados48h, 10000);
      setInterval(limparDados48h, 48 * 60 * 60 * 1000);
    })
