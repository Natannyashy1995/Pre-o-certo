...
app.post('/api/ia/analisar', iaLimiter, async (req, res) => {
  ...
  try {
    ...
    const modelos = ['gemini-2.5-flash', 'gemini-2.0-flash', 'gemini-1.5-flash', 'gemini-1.5-flash-latest', 'gemini-2.0-flash-lite'];
    let ultimoErro = '';
    ...
    for (const modelo of modelos) {
      try {
        ...
        if (!texto) { ultimoErro = 'Resposta vazia do modelo ' + modelo; continue; }
...
app.delete('/api/precos/:produtoId/:mercadoId', adminAuth, async (req, res) => {
  try {
    ...
    if (!isObjId(produtoId) || !isObjId(mercadoId)) return res.status(400).json({ erro:'IDs inválidos' });
    await Preco.findOneAndDelete({ produtoId, mercadoId });
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ erro: e.message }); }
});
...
app.post('/api/solicitacao-produto', authMiddleware, async (req, res) => {
  try {
    const { nome, preco, obs, autor } = req.body;
    if (!nome) return res.status(400).json({ erro:'Nome do produto obrigatório' });
    // Registra como log para o admin ver
    await registrarLog('produto_solicitado', `Novo produto solicitado: "${nome}" | Preço sugerido: R$ ${preco||'N/A'} | Por: ${autor||req.user.login||'anon'}`, req.user.login||req.user.usuario||'anon', getIP(req));
    // Notifica admins
    notificarAdmins('produto_solicitado', { nome, preco, autor: autor||req.user.login });
